...
class ReportGenerator(private val context: Context) {
    ...
    private fun drawProbandPage(
        ...
        // Column 3: Diagnostic Score
        canvas.drawText("Diagnostic Score", 430f, 365f, statLabelPaint)
        val scoreColor = when {
        ...
        canvas.drawText(String.format(Locale.US, "%.1f %%", proband.stats.diagnosticScore), 430f, 395f, statValuePaint)
        statValuePaint.color = Color.rgb(15, 23, 42) // reset

        // Column 4: Status Indicator
        canvas.drawText("Klinischer Status", 620f, 365f, statLabelPaint)
        statValuePaint.color = scoreColor
        ...
        canvas.drawText(statusLabel, 620f, 395f, statValuePaint)
        statValuePaint.color = Color.rgb(15, 23, 42) // reset
    ...
    private fun calculateStats(data: List<Float>): ClinicalStats {
        ...
        val interpretation = when {
            diagnosticScore > 65f -> "PATHOLOGISCH: Hohe multivariate kognitive Instabilität. Schizophrenie-Signatur detektiert."
            diagnosticScore > 35f -> "GRENZWERTIG: Sporadische Instabilitäten und neuronale Belastungsepisoden."
            else -> "STABIL: Keine physiologischen Auffälligkeiten. Trajektorie verläuft vollständig normal."
        }

        return ClinicalStats(mean, variance, maxVal, crossings, rmsd, timeAbovePercent, diagnosticScore, interpretation)
    }
    ...
    private fun renderTrajectoryToBitmap(data: List<Float>, label: String, stats: ClinicalStats): Bitmap {
        ...
        canvas.drawText("Diag. Score: ${String.format("%.1f%%", stats.diagnosticScore)}", 500f, 155f, textPaint)
