...
class EegViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun calculateStats(data: List<Float>, type: String): ReportGenerator.ClinicalStats {
        ...
        return ReportGenerator.ClinicalStats(mean, variance, maxVal, crossings, rmsd, timeAbovePercent, diagnosticScore, interpretation)
    }
    ...
    private fun serializeBatchReport(probands: List<ReportGenerator.ProbandReportData>): String {
        ...
        sb.append("#BATCH_REPORT_V2\n")
        probands.forEach { proband ->
            val dataPointsStr = proband.dataPoints.joinToString(",") { String.format(Locale.US, "%.3f", it) }
            sb.append("${proband.id};${proband.name};${proband.type};${proband.stats.mean};${proband.stats.variance};${proband.stats.maxVal};${proband.stats.crossings};${proband.stats.rmsd};${proband.stats.timeAbovePercent};${proband.stats.diagnosticScore};${proband.stats.interpretation};$dataPointsStr\n")
        }
        return sb.toString()
    }
            ...
            try {
                ...
                if (isV2 && parts.size >= 12) {
                    ...
                    ReportGenerator.ProbandReportData(
                        ...
                        stats = ReportGenerator.ClinicalStats(mean, variance, maxVal, crossings, rmsd, timeAbovePercent, diagnosticScore, interpretation)
                    )
